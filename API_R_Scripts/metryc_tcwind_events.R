...
dir.create("output/metryc_tcwind_events", recursive = TRUE)
...
metryc_tcwind_events_api_request <- function(lat, lon, index) {
  query_params <- list(
    lat = lat,
    lon = lon,
    time_horizon = "now",
...
for (i in locations_dt$index) {
  ...
  metryc_tcwind_events_api_request(lat, lon, i)
}
...
output_excel_file <- "output/Reask_Metryc_TCWind_Events.xlsx"
