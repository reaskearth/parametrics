##############################################################################################
# Reask Cat in a Circle API Query Script for Probabilistic Track Data (based on DeepCyc)
##############################################################################################
#
...
#
# Description:
#   This script queries the Reask API /deepcyc/tctrack/events endpoint for maximum wind speed data
#   (Vmax) used to structure and price Cat in a Circle parametric contracts.
#
...
#   mapped back to the original loc_id). The CSV and Parquet formats include the data in long 
...
# Output file names:
output_xlsx                    <- "results/deepcyc_tctracks_events_CIC.xlsx"
output_csv                     <- "results/deepcyc_tcwind_events_long_CIC.csv"
output_parquet                 <- "results/deepcyc_tcwind_events_long_CIC.parquet"
...
for (i in 1:nrow(locations)) {
  ...
  dt_list <- lapply(radius_IDs, function(radius_ID) {
    ...
    dt <- data.table(
      event_id = Events$features$properties$event_id,
      year_id = Events$features$properties$year_id,
      loc_idx = i,
...
arrow::write_parquet(all_data, output_parquet)
...
for (loc in unique_locations) {
  ...
  pivot_loc <- dcast(
    ...
    event_id + year_id ~ paste0("Radius_",radius_ID),
...
print("Processing DeepCyc complete! Results saved in the 'results' folder.")
